...
Message delivery isn't guaranteed to be in order. An older update can arrive after a newer one, and if we blindly apply every event, the data can end up reverting to an older state.

## Solution

Every update carries a version generated by the source service. The consumer only accepts an update if its version is higher than the one already stored.
...
This single query covers both insert and update. Older versions are simply ignored.

```go
tag, err := r.pool.Exec(ctx, ..., update.TicketID, update.Quantity, update.Version)
return tag.RowsAffected() == 1, nil
```

If `RowsAffected()` is `0`, it just means the incoming event was older than the current state. That's expected, not an error.

## Why Version Instead of Timestamp?

Timestamps depend on server clocks being in sync. A version generated by the source gives a consistent ordering without relying on time.

## Trade-offs

| Decision | Reason |
| --- | --- |
| Use version numbers | Reliable ordering without clock sync issues |
| Single UPSERT query | Atomic, no extra existence check |
| Ignore older events | Out-of-order delivery is normal, so no need to treat it as an error |

---

# Combined Scenario Flowchart

```mermaid
flowchart TB
    subgraph CHAIN["Ticket Purchase Chain (Section 1 → 2 → 3)"]
        direction TB
        A([Purchase request]) --> B["Sec 1: UPDATE stock = stock - 1<br/>WHERE stock > 0"]
        B --> C{rows_affected == 1?}
        C -->|No| C1([409 Sold Out])
        C -->|Yes| D[Enqueue transaction]
        D --> D1([200 OK to client])
        D --> E([Sec 2: consumer dequeues])
        E --> F[Attempt DB save]
        F --> G{Saved?}
        G -->|No| H{Retries exhausted?}
        H -->|No| I[Backoff + retry]
        I --> F
        H -->|Yes| J[Write to dead-letter table]
        G -->|Yes| K["Insert transactions row<br/>+ outbox row (same tx)"]
        K --> L([Sec 3: publisher polls outbox])
        L --> M{Breaker open?}
        M -->|Yes| N([Skip, retry next poll])
        M -->|No| O[Send to accounting API]
        O --> P{Delivered?}
        P -->|Yes| Q([Mark sent])
        P -->|No| R([Mark failed, record breaker failure])
    end

    subgraph WEBHOOK["Section 4: Inbound Webhook (independent)"]
        direction TB
        W1([Webhook payload received]) --> W2["Redis SETNX(payment_id)"]
        W2 --> W3{Key existed?}
        W3 -->|Yes| W4([200 OK, idempotent no-op])
        W3 -->|No| W5["INSERT ... ON CONFLICT<br/>DO NOTHING"]
        W5 --> W6([200 OK])
    end

    subgraph SYNC["Section 5: Availability Sync (independent)"]
        direction TB
        S1([Availability update received]) --> S2["INSERT ... ON CONFLICT<br/>DO UPDATE WHERE version < incoming"]
        S2 --> S3{rows_affected == 1?}
        S3 -->|Yes| S4([applied: true])
        S3 -->|No| S5([Log discarded]) --> S6([applied: false])
    end
    ...
    classDef outbox fill:#ccfbf1,stroke:#14b8a6,color:#134e4a;
    classDef webhook fill:#fee2e2,stroke:#ef4444,color:#7f1d1d;
    classDef sync fill:#ede9fe,stroke:#8b5cf6,color:#4c1d95;

    class A,B,C,C1,D,D1,E,F,G,H,I,J chain
    class K,L,M,N,O,P,Q,R outbox
    class W1,W2,W3,W4,W5,W6 webhook
    class S1,S2,S3,S4,S5,S6 sync
```